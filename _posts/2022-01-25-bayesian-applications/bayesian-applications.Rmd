...
library(rstanarm)     # interface for stan platform, regression applications
...
library(EpiNow2)      # epidemiological model with stan backend
```
...
```{r Heart final regression - summary}
...
```{r Heart rstanarm glm, results='hide', message=FALSE}
...
Covid case and test data sourced from the department of [NSW Website](https://data.nsw.gov.au/) on 17/01/2022. Cases:
...
There are some pragmatic methods for adjusting the case numbers for [increasing test positivity rates](http://freerangestats.info/blog/2020/05/09/covid-population-incidence). For our purposes we will focus the model on case data before 23 December 2021 in order to exclude the period where the test data became less reliable/ complete.
...
![](img/covid_estimates.jpg){width=100%}
