...
![Golf in boosting](golf-gbdt.png)
...
* XGBoost can handle missing values. Common treatments are to: 1) use median values, or 2) do so but also consider the similarity of the data point against all others (proximity measures). XGBoost treats missing values as a sparse matrix. When splitting, the missing value is calculated in both the left and right tree, and will be assigned to whichever side gives better loss reduction (clever isn't it). This assumes that the training data and predicting data have same distribution.
